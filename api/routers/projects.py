...
def get_project_detail(
    ...
    """Get project details."""
    if not check_permission(db, str(user.id), project_id):
...
def update_project_detail(
    ...
    if not check_permission(db, str(user.id), project_id, "admin"):
...
def delete_project(
    ...
    if not soft_delete_project(db, project_id):
        raise HTTPException(status_code=404, detail="项目不存在")
...
def restore_deleted_project(
    ...
    if not restore_project(db, project_id):
...
def permanent_delete_project(
    ...
    if not hard_delete_project(db, project_id):
        raise HTTPException(status_code=404, detail="项目不存在")
    return {"status": "permanently_deleted", "project_id": project_id}
...
def invite_member(
    ...
    try:
        return add_member(db, project_id, req.email, req.role)
...
def change_member_role(
    ...
    try:
        if not update_member_role(db, project_id, member_user_id, req.role):
...
def kick_member(
    ...
    if not remove_member(db, project_id, member_user_id):
        raise HTTPException(status_code=404, detail="成员不存在")
