        ...
        {importResult ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => { setInputMode("file"); setIsParsed(false) }}
              ...
              <button
                onClick={() => { setInputMode("text"); setIsParsed(false) }}
                className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${
                  inputMode === "text"
                    ? "bg-primary text-primary-foreground"
                    : "border border-border bg-background hover:bg-muted"
            ...
            <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
              ...
              <code className="block whitespace-pre font-mono">{TEMPLATE_CONTENT}</code>
            </div>

            {/* 文件上传 */}
            {inputMode === "file" && (
              <div
                className="flex flex-col items-center justify-center gap-3 rounded-lg border-2 border-dashed border-border bg-muted/20 px-6 py-8 cursor-pointer hover:border-primary/50 transition-colors"
              ...
              >
                ...
                {csvText && (
                  <p className="text-xs text-green-600">已读取文件，共 {previewRows.length} 行数据</p>
                )}
                <input
                  ref={fileInputRef}
                  type="file"
            ...
            {inputMode === "text" && (
              <div className="space-y-2">
                ...
                />
                <Button variant="outline" size="sm" onClick={handleTextParse} disabled={!csvText.trim()}>
                  解析预览
                </Button>
              </div>
            )}
            ...
            {isParsed && previewRows.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm text-muted-foreground">
                  ...
                  {hasErrors && (
                    <span className="ml-1 text-amber-600">（红色行将被跳过）</span>
                  )}
                </p>
                    ...
                    <TableBody>
                      {previewRows.map((row) => (
                        ...
                        >
                          ...
                          <TableCell className="max-w-[120px] truncate">{row.desc || "—"}</TableCell>
                          <TableCell>
                            {row.error ? (
            ...
            {isParsed && previewRows.length === 0 && (
              <p className="text-sm text-muted-foreground text-center py-4">未解析到任何数据行</p>
            )}
