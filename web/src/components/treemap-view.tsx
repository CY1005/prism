    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1">
            <span className="h-2.5 w-2.5 rounded-sm bg-rose-100 border border-rose-300" />
            完善度 &lt; 40%
          </span>
          <span className="flex items-center gap-1">
      ...
      <div className="relative min-h-[500px]">
        ...
        {data.length === 0 ? (
          ...
          <div className="grid grid-cols-5 auto-rows-[140px] gap-3">
            {data.map((item) => {
              const sizeClass = getBlockSize(Math.max(item.featureCount, 1), totalFeatures)
              return (
                <div
                  key={item.nodeId}
                  className={`${sizeClass} ${completionToColorClass(item.completionPercent)} rounded-lg border-2 p-5 cursor-pointer transition-all flex flex-col justify-between hover:shadow-md`}
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className={`text-2xl font-bold ${completionToTextColor(item.completionPercent)}`}>
                      {item.completionPercent}%
                    </span>
                    <span className={`h-2 w-2 rounded-full ${completionToDot(item.completionPercent)}`} />
                  </div>
                </div>
              )
            })}
