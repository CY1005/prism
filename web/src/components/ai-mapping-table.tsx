...
export function AIMappingTable({
  rows,
  folders,
  dimensions,
  onChange,
  ...
  const [filterConfidence, setFilterConfidence] = useState<Confidence | "all">(
    "all",
  );
  ...
  const bulkUpdateModule = (moduleId: string | null) => {
    ...
    onChange(
      rows.map((r) =>
        r.selected ? { ...r, override_module_id: moduleId } : r,
      ),
    );
  };
  ...
  const bulkUpdateDimension = (dimId: string | null) => {
    ...
    onChange(
      rows.map((r) =>
        r.selected ? { ...r, override_dimension_id: parsed } : r,
      ),
    );
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b bg-muted/20 gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <Select onValueChange={bulkUpdateDimension}>
            <SelectTrigger className="w-[150px] h-7 text-sm">
          ...
          <TableBody>
            {displayRows.map((row) => {
              const effectiveModuleId =
                row.override_module_id ?? row.recommended_module_id;
                ...
                >
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(v) =>
                        v && updateRow(row.id, { override_module_id: v })
                      }
                    >
                  ...
                  <TableCell>
                    <Select
                      value={
                        effectiveDimId != null
                          ? String(effectiveDimId)
                          : "none"
                      }
                      onValueChange={(v) =>
                        updateRow(row.id, {
                          override_dimension_id:
