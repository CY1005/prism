    ...
    <div className="space-y-4">
      ...
      {references.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center border border-dashed rounded-md">
          <Building className="h-10 w-10 text-muted-foreground/40 mb-2" />
          <p className="text-sm text-muted-foreground">暂无竞品参考，点击添加</p>
        </div>
      ) : (
        <div className="space-y-4">
          {references.map((ref) => (
            <div key={ref.reference.id} className="rounded-md border border-border p-4 group">
              <div className="flex items-center gap-2 mb-2">
                <span className="font-medium text-sm">{ref.competitor.name}</span>
                {ref.reference.version && (
                  <Badge variant="outline" className="text-xs">{ref.reference.version}</Badge>
                )}
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => onEdit(ref)}>
              ...
              {ref.reference.featureCoverage && (
                <p className="text-sm text-muted-foreground mb-3">{ref.reference.featureCoverage}</p>
              )}

              {ref.reference.technicalApproach && (
                <p className="text-sm text-muted-foreground mb-3">
                  <span className="text-xs font-medium text-foreground">技术方案：</span>
                  {ref.reference.technicalApproach}
                </p>
              )}

              {ref.reference.prosAndCons &&
                ((ref.reference.prosAndCons.pros?.length ?? 0) > 0 || (ref.reference.prosAndCons.cons?.length ?? 0) > 0) && (
...
interface AddReferenceDialogProps {
  ...
  competitors: Competitor[];
  onCreateCompetitor: (data: { name: string; website?: string; description?: string }) => Promise<string | null>;
  onSubmit: (data: {
    competitorId: string;
    version?: string;
...
export function AddReferenceDialog({
  ...
  const [featureCoverage, setFeatureCoverage] = useState(editingRef?.reference.featureCoverage ?? "");
  const [technicalApproach, setTechnicalApproach] = useState(editingRef?.reference.technicalApproach ?? "");
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            {pros.length > 0 && (
              <div className="flex gap-1 flex-wrap">
                {pros.map((p, i) => (
                  <Badge key={i} variant="secondary" className="text-xs gap-1 bg-green-50 text-green-700">
                    {p}
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setPros(pros.filter((_, j) => j !== i))} />
                  </Badge>
                ))}
              </div>
            )}
          ...
          <div className="space-y-2">
            ...
            {cons.length > 0 && (
              <div className="flex gap-1 flex-wrap">
                {cons.map((c, i) => (
                  <Badge key={i} variant="secondary" className="text-xs gap-1 bg-red-50 text-red-700">
                    {c}
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setCons(cons.filter((_, j) => j !== i))} />
                  </Badge>
                ))}
              </div>
            )}
...
interface CompetitorManagementProps {
  ...
  onUpdateCompetitor: (id: string, data: { name?: string; website?: string; description?: string }) => void;
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">管理项目级全局竞品实体，确保名称统一、跨功能项复用</p>
        </div>
        <Button onClick={openCreate} disabled={!canAdmin} title={!canAdmin ? "查看者无编辑权限" : undefined}>
      ...
      {competitors.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed rounded-md">
          ...
          <p className="text-sm text-muted-foreground">暂无竞品，点击添加</p>
        </div>
      ) : (
        <div className="space-y-3">
          {competitors.map((c) => (
            <div key={c.id} className="flex items-center gap-4 p-4 rounded-md border border-border group">
      ...
      <Dialog open={addDialog} onOpenChange={setAddDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>名称</Label>
              <Input value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="竞品名称" autoFocus />
            </div>
            <div className="space-y-2">
              <Label>网站（可选）</Label>
            ...
            <div className="space-y-2">
              <Label>描述（可选）</Label>
              <Textarea value={formDescription} onChange={(e) => setFormDescription(e.target.value)} placeholder="简要描述" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={editDialog} onOpenChange={setEditDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdate} disabled={!formName.trim()}>保存</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
