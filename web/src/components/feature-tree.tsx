    ...
    <div>
      ...
      >
        ...
        {isEditing ? (
          <input
            ref={inputRef}
            className="flex-1 rounded border bg-background px-1 py-0.5 text-sm outline-none focus:ring-1 focus:ring-ring"
            value={editingName}
...
function RootContextMenu({
  ...
  return (
    ...
    >
      <button
        className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent"
        onClick={() => { onAddChild?.(null, "folder"); onClose(); }}
      >
        <Folder className="h-4 w-4" /> 添加顶层文件夹
      </button>
      <button
        className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent"
        onClick={() => { onAddChild?.(null, "file"); onClose(); }}
      >
...
export function FeatureTree({ data, selectedId, onSelect, onAddChild, onRename, onDelete, onReorder, onMove }: FeatureTreeProps) {
  ...
  return (
    <div className="space-y-1 py-2 min-h-[100px]" onContextMenu={handleEmptyAreaContextMenu}>
      {data.map((node) => (
        <TreeItem
          key={node.id}
          node={node}
