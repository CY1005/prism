...
export function AnalysisResult({ layer }: AnalysisResultProps) {
  const directModules = layer.affected_modules.filter(
    (m) => m.impact_level === "high",
  )
  const indirectModules = layer.affected_modules.filter(
    (m) => m.impact_level !== "high",
  )
    ...
    <div className="space-y-4">
      ...
      {layer.metadata && layer.isComplete && (
        <div className="flex items-center gap-4 text-xs text-muted-foreground pt-1">
          ...
          {layer.metadata.tokens_used > 0 && (
            <span>Token: {layer.metadata.tokens_used}</span>
          )}
        </div>
      )}
    </div>
