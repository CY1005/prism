...
export async function updateTeam(
  ...
  try {
    ...
    if (team.ownerId !== user.id) {
      ...
      if (!member) {
        return actionError(
          new AppError("无权限修改团队", "blocking", "FORBIDDEN", 403),
        );
      }
    }
...
export async function deleteTeam(teamId: string): Promise<ActionResult> {
  try {
    ...
    if (team.ownerId !== user.id) {
      return actionError(
        new AppError("只有团队创建者可以删除团队", "blocking", "FORBIDDEN", 403),
      );
    }
...
export async function removeMember(
  ...
  try {
    ...
    if (userId === team.ownerId) {
      return actionError(
        new AppError("不能移除团队创建者", "blocking", "VALIDATION_ERROR"),
      );
    }
    ...
    if (team.ownerId !== user.id) {
      ...
      if (!member) {
        return actionError(
          new AppError("无权限移除成员", "blocking", "FORBIDDEN", 403),
        );
      }
    }
...
export async function updateMemberRole(
  ...
  try {
    ...
    if (team.ownerId !== user.id) {
      return actionError(
        new AppError("只有团队创建者可以修改角色", "blocking", "FORBIDDEN", 403),
      );
    }

    if (userId === team.ownerId) {
      return actionError(
        new AppError("不能修改团队创建者的角色", "blocking", "VALIDATION_ERROR"),
      );
    }
    ...
    if (!validRoles.includes(role)) {
      return actionError(
        new AppError("无效的角色", "blocking", "VALIDATION_ERROR"),
      );
    }
...
export async function migrateProjectToTeam(
  ...
  try {
    ...
    if (teamId) {
      ...
      if (!isMember) {
        return actionError(
          new AppError("你不是该团队成员", "blocking", "FORBIDDEN", 403),
        );
      }
    }
