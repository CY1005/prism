...
export async function createVersion(
  ...
  try {
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(versionRecords.nodeId, nodeId),
          eq(versionRecords.versionLabel, versionLabel),
        ),
      );
    ...
    const [newVersion] = await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(
          and(
            eq(versionRecords.nodeId, nodeId),
            eq(versionRecords.isCurrent, true),
          ),
        );
...
export async function deleteVersion(
  ...
  try {
    ...
    if (!version)
      return actionError(
        new AppError("版本记录不存在", "blocking", "NOT_FOUND", 404),
      );

    const [node] = await db
      .select()
      .from(nodes)
      .where(eq(nodes.id, version.nodeId));
    if (!node)
      return actionError(
