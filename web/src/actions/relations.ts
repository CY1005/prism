...
export async function createRelation(data: {
  ...
  try {
    ...
    if (data.sourceNodeId === data.targetNodeId) {
      return actionError(
        new AppError("不能创建自引用关联", "blocking", "VALIDATION_ERROR", 400),
      );
    }
    ...
    const [sourceNode] = await db
      .select()
      .from(nodes)
      .where(eq(nodes.id, data.sourceNodeId));
    if (!sourceNode) {
      return actionError(
        new AppError("源节点不存在", "blocking", "NOT_FOUND", 404),
      );
    }
    ...
    const [targetNode] = await db
      .select()
      .from(nodes)
      .where(eq(nodes.id, data.targetNodeId));
    if (!targetNode) {
      return actionError(
        new AppError("目标节点不存在", "blocking", "NOT_FOUND", 404),
      );
    }
...
export async function deleteRelation(
  ...
  try {
    ...
    if (!relation) {
      return actionError(
        new AppError("关联不存在", "blocking", "NOT_FOUND", 404),
      );
    }
    ...
    const [sourceNode] = await db
      .select()
      .from(nodes)
      .where(eq(nodes.id, relation.sourceNodeId));
    if (sourceNode) {
      await checkProjectAccess(user.id, sourceNode.projectId, "editor");
    }
...
export async function getModuleRelationDetail(
  ...
  try {
    ...
    const [moduleNode] = await db
      .select()
      .from(nodes)
      .where(eq(nodes.id, moduleNodeId));
    if (!moduleNode) {
      return actionError(
        new AppError("模块不存在", "blocking", "NOT_FOUND", 404),
      );
    }
