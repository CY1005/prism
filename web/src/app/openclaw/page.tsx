...
const versionData = [
  {
    ...
    details: "优化了日报技能的定时任务调度，修复了因时区问题导致的cron执行异常。同时升级路由层至v2架构，支持更灵活的意图分发。",
  },
  {
    version: "2026-03-28",
    summary: "新增人格记忆系统，支持多人格切换",
    details: "引入人格记忆机制，支持在对话中切换不同的AI人格，每个人格保持独立的记忆上下文和对话风格。",
  },
  {
    version: "2026-03-15",
      ...
      >
        <div className="flex h-14 items-center justify-between border-b border-sidebar-border px-4">
          <div className="flex items-center gap-2">
            <Link href="/projects" className="font-semibold text-sidebar-foreground hover:text-primary transition-colors">OpenClaw</Link>
        ...
        <ScrollArea className="flex-1">
          <div className="px-2">
            <ProtoFeatureTree
              ...
              defaultExpanded={["message-layer", "telegram-bridge", "router-layer", "fastapi-router", "skill-layer", "memory-layer"]}
            />
          </div>
        </ScrollArea>
      </aside>
          ...
          <div className="mx-auto max-w-4xl space-y-4 p-6">
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell className="font-mono text-xs">api.telegram.org/bot{"{token}"}/getUpdates</TableCell>
                      <TableCell className="text-muted-foreground">长轮询接收消息</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">出站</TableCell>
                      <TableCell>HTTP</TableCell>
                      <TableCell className="font-mono text-xs">localhost:8900/webhook/telegram</TableCell>
                      <TableCell className="text-muted-foreground">转发至Router处理</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">出站</TableCell>
                      <TableCell>HTTPS</TableCell>
                      <TableCell className="font-mono text-xs">api.telegram.org/bot{"{token}"}/sendMessage</TableCell>
            ...
            >
              <div className="rounded-md border border-border bg-amber-50/50 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-foreground">Telegram API 限流踩坑</h4>
                    <p className="mt-1 text-sm text-muted-foreground">
                      getUpdates 频率超过 30次/秒会被临时封禁 token。根因：轮询间隔设得太短。修复：间隔从 0.5秒 改为 2秒，增加 429 状态码自动退避。
                    </p>
                    <div className="mt-3 flex flex-wrap gap-1">
                      ...
                      <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100">Telegram</Badge>
                    </div>
                  </div>
                </div>
              </div>
            ...
            >
              <div className="rounded-md border border-border bg-muted/30 p-4">
                <h4 className="text-sm font-medium text-foreground">systemd 服务配置</h4>
                <p className="mt-1 text-sm text-muted-foreground">
                  服务名 telegram-polling.service，开机自启（WantedBy=multi-user.target），异常退出 3秒后自动重启（RestartSec=3）。依赖 message-router.service 先启动。
                </p>
