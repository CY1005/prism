...
export const Errors = {
  ...
  ACCOUNT_DISABLED: new AppError('账号已被禁用，请联系管理员', 'blocking', ErrorCode.ACCOUNT_DISABLED, 403),
  ACCOUNT_LOCKED: new AppError('账号已锁定，请稍后重试', 'blocking', ErrorCode.ACCOUNT_LOCKED, 423),

  // 数据冲突 (blocking)
  VERSION_CONFLICT: new AppError('内容已被他人修改，请刷新后重试', 'blocking', ErrorCode.VERSION_CONFLICT, 409),
  ...
  NOT_FOUND: (resource: string) => new AppError(`${resource}不存在`, 'blocking', ErrorCode.NOT_FOUND, 404),

  // 外部服务 (warning)
  AI_SERVICE_UNAVAILABLE: new AppError('AI分析服务暂时不可用，请稍后重试', 'warning', ErrorCode.AI_UNAVAILABLE, 503),
