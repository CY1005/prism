...
## BUG-104: 全局页面标题显示 "Create Next App"
...
所有页面的浏览器标签标题显示 "Create Next App" 而不是产品名称。

### 根因

`web/src/app/layout.tsx` 中的 `metadata` 对象仍然是 Next.js 脚手架生成的默认值，从未修改过：
```tsx
...
将标题改为 "Prism — 产品知识管理平台"，描述改为 "以功能模块为中心的产品知识管理平台"。
...
1. **脚手架默认值是定时炸弹**：项目从 `create-next-app` 初始化后，默认 metadata 很容易被忽略，直到上线才被发现。应在项目初始化 checklist 中包含"修改默认 metadata"。
2. **E2E 测试应包含页面标题验证**：自动化测试用例应检查 `<title>` 标签内容。

## BUG-105: 5 个页面 header 显示硬编码 mock 数据（用户名/项目名）
...
overview、analysis、comparison、issues、product-lines 五个页面的 header 区域始终显示：
- 用户名："陈琦"（mock）
- 项目名称："AI云平台竞品分析"（mock）
- 面包屑导航中的项目名也是 mock

无论实际登录用户是谁、当前项目是什么，都显示相同的 mock 数据。

### 根因

这 5 个 `"use client"` 页面都从 `@/lib/project-detail-data` 导入 `detailStrings` 对象，该对象包含硬编码的 mock 字符串：
```tsx
...
这是开发初期用于 UI 原型的 mock 数据，在实现真实数据流后从未被替换。

### 修复

1. 创建 `web/src/lib/use-page-context.ts` 共享 Hook，通过 Server Action（`getProject` + `getSessionUser`）获取真实项目名和用户信息
2. 5 个页面统一：移除 `detailStrings` 导入，使用 `usePageContext(projectId)` Hook
3. 面包屑中的项目名改为动态获取，加载中显示"加载中..."
4. 用户 avatar/名称改为实际登录用户数据

### 受影响文件

- `web/src/lib/use-page-context.ts`（新建）
- `web/src/app/projects/[projectId]/overview/page.tsx`
- `web/src/app/projects/[projectId]/analysis/page.tsx`
- `web/src/app/projects/[projectId]/comparison/page.tsx`
- `web/src/app/projects/[projectId]/issues/page.tsx`
- `web/src/app/projects/[projectId]/product-lines/[plId]/page.tsx`

### 教训

1. **Mock 数据应有明确的生命周期**：引入 mock 时应在代码中标注 `// TODO: replace with real data`，或在 tech debt tracker 中记录。否则很容易在多个页面复制传播后被遗忘。
2. **UI 原型和生产代码的分界**：当 mock 数据作为 import 在多个文件间共享时，它已经从"临时方案"变成了"架构问题"。应尽早创建数据获取的标准模式（如共享 Hook），避免每个页面各自硬编码。
3. **系统性审查**：发现一个页面有 mock 数据后，应 `grep` 搜索所有使用同一 mock 数据源的页面，一次性修复。

## BUG-106: Overview 页面"最近更新"侧边栏显示硬编码假数据
...
Overview 页面右侧"最近更新"区域始终显示 5 条固定的假活动记录（"陈 更新了 推理服务>创建推理服务 的技术实现 / 2小时前"等），与实际项目活动无关。

### 根因

`overview/page.tsx` 第 243 行直接使用 `@/lib/project-detail-data` 导出的 `recentUpdates` 数组，这是纯粹的 mock 数据。讽刺的是，同一个页面已经实现了完整的活动日志面板（F15），使用 `getActivityLogs` Server Action 获取真实数据，但侧边栏的"最近更新"却仍用 mock。

### 修复

提取 `RecentUpdatesSidebar` 组件，内部调用 `getActivityLogs(projectId, 1, 5)` 获取最近 5 条真实活动记录：
- 有数据：显示活动摘要 + 日期
- 无数据：显示"暂无更新记录"
...
1. **同一页面的不同区域可能有不同的数据源**：页面主体区域（全景图、统计卡片）已用真实数据，但侧边栏小组件被遗漏。Code Review 时应检查页面的每个数据渲染点。
2. **已有 Server Action 但未复用**：`getActivityLogs` 已经存在且可用，但侧边栏没有调用它。开发新功能时应先检查是否有可复用的现有接口。

## BUG-107: Overview 页面统计/树形数据客户端 fetch 在公网环境下失败
...
Overview 页面的统计卡片（产品线数/模块数/功能项/完善度）和树形视图永远停留在"加载中..."状态，无法显示真实数据。

### 根因

`web/src/services/project-stats.ts` 中的 `getProjectStats()` 和 `getProjectTreeOverview()` 在**浏览器端**直接 fetch FastAPI：
```tsx
...
公网环境下 `NEXT_PUBLIC_ANALYZER_URL` 设为 `https://api-prism.19911005.xyz`，但：
1. FastAPI 的 `/api/projects/{id}/stats` 需要 `require_user` 认证（Internal Token + User-Id header）
2. 客户端 fetch 不携带这些 header
3. 返回 `{"detail":"未登录"}`，被 `resp.ok` 判断为失败

这是 BUG-101/103 的同类问题：`"use client"` 组件不应直调内部微服务。

### 修复

1. 创建 `web/src/actions/project-stats-proxy.ts` Server Action，在服务端调用 FastAPI，携带 `X-Internal-Token` 和 `X-User-Id`
2. `overview/page.tsx` 中将 `getProjectStats()` 替换为 `getProjectStatsAction()`，`getProjectTreeOverview()` 替换为 `getProjectTreeOverviewAction()`

### 受影响文件

- `web/src/actions/project-stats-proxy.ts`（新建）
- `web/src/app/projects/[projectId]/overview/page.tsx`
...
## BUG-109: 空项目导入文档时无法选择目标模块（鸡生蛋问题）
...
新建项目后首次导入文档，在第 3 步"映射配置"中，"目标模块"下拉列表为空，无法选择任何模块。用户被卡死在这一步，无法完成导入。

三种导入模式（手动映射、AI 智能导入、Markdown 导入）都受影响。

### 根因

导入流程要求每个文件映射到一个已有的 `folder` 类型节点（模块）作为父节点。但新项目没有任何节点，`folders` 数组为空。

```tsx
// page.tsx — 服务端预取 folders
const tree = await getProjectTree(projectId); // 新项目返回 []
const folders = collectFolders(tree); // → []

// import-wizard.tsx — mapping 初始化
targetNodeId: folders[0]?.id ?? "", // → ""

// 确认导入时
const selected = mappingRows.filter((r) => r.selected && r.targetNodeId);
// targetNodeId 为空 → 全部被过滤掉 → "请至少选择一个文件并指定目标模块"
```

这是一个**流程设计缺陷**：用户被要求选择还不存在的东西。

### 修复

**核心思路**：上传 ZIP 后，如果项目为空，从 ZIP 的目录结构自动创建模块。

1. **新增 `createModulesFromZipTree` Server Action**（`actions/import.ts`）：
   - 读取 ZIP 的 `FileTreeNode` 结构
   - 为每个顶级目录创建 `folder` 类型节点（支持 2 层：产品线 → 模块）
   - 如果 ZIP 没有目录（所有文件在根目录），创建一个默认"导入文档"模块
   - 返回新创建的 folders 列表

2. **ImportWizard 增加 `dynamicFolders` 状态**：
   - `useState<FlatFolder[]>(folders)` 初始值为 props 传入的 folders
   - 上传 ZIP 后检测：`if (dynamicFolders.length === 0)` → 调用 `createModulesFromZipTree`
   - 更新 `dynamicFolders` 和 mapping 目标

3. **智能自动映射**：
   - 根据文件在 ZIP 中的父目录名，自动匹配对应的新建模块
   - 例如 `推理服务/创建推理服务.md` → 自动映射到"推理服务"模块

### 受影响文件

- `web/src/actions/import.ts`（新增 `createModulesFromZipTree`）
- `web/src/app/projects/[projectId]/import/import-wizard.tsx`（`dynamicFolders` + 自动建模块逻辑）
